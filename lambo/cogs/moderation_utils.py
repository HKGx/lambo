...
class ModerationUtilsCog(Cog, name="Moderation Utilities"):
    ...
    def set_channel_overwrites(self, overwrites: discord.PermissionOverwrite, permissions: PermissionsFor) -> discord.PermissionOverwrite:
        """
        SIDE EFFECTS!
        """
    ...
    def check_bot_perms_for_channel(self, ctx: Context, channel: discord.abc.GuildChannel, permission: discord.flags.flag_value) -> bool:
    ...
    async def channel_permissions_add(self,
                                      ctx: Context,
                                      channel: typing.Union[discord.TextChannel, discord.CategoryChannel, discord.VoiceChannel, discord.StageChannel],
                                      member: discord.Member,
                                      *,
                                      permissions: PermissionsFor):
        assert isinstance(
            ctx.author, discord.Member), "ctx.author is not a discord.Member"
        if not channel.permissions_for(ctx.author).manage_permissions:
            await ctx.send("You do not have permission to manage permissions for this channel.")
            return
        await ctx.send(f"Adding permissions for {member.name} in {channel}. It might take a while...")
        ctx.typing()
        permissions_set_for: set[str] = set()
        permissions_lacking_for: set[str] = set()
        if not self.check_bot_perms_for_channel(ctx, channel, discord.Permissions.manage_permissions):
            ...
            try:
                ...
                await ctx.send(f"Failed to set permissions for {member.name} in {channel} : {e}")
                permissions_lacking_for.add(channel.mention)
        if isinstance(channel, discord.CategoryChannel):
            for subchannel in channel.channels:
                ...
                try:
                    ...
                    await ctx.send(f"Failed to set permissions for {member.name} in {subchannel.name} : {e}")
                    permissions_lacking_for.add(subchannel.mention)

        await ctx.reply(f"Set permissions for {member.name} in {', '.join(permissions_set_for)}\n"
